...
@time_logger
def export_to_wds(writer, audio, asr_result, file_name, json_data):
    sr = audio["sample_rate"]
    audio = audio["waveform"]

    for idx, segment in tqdm.tqdm(enumerate(asr_result),
                                  total=len(asr_result),
                                  desc="Exporting to webdataset"):
        ...
        json_merge["emilia_pipe"] = json_wds

        example = {
            "__key__": uttid,
            "mp3": buffer.getvalue(),
            "json": json.dumps(json_merge, ensure_ascii=False),
        }
...
def filter(mos_list, min_dnsmos):
    """
    ...
    """
    filtered_audio_stats, all_audio_stats = calculate_audio_stats(mos_list, min_dnsmos=min_dnsmos)
...
def main_process_wds(sample, writer=None, **kwargs):
    """
    Process the audio file, including standardization, source separation, speaker segmentation, VAD, ASR, export to MP3, and MOS prediction.
    ...
    logger.info("[SKIP] Step 2: Speaker Diarization")
    #speakerdia = speaker_diarization(audio)

    logger.info("[SKIP] Step 3: Fine-grained Segmentation by VAD")
    #vad_list = vad.vad(speakerdia, audio)
    #segment_list = cut_by_speaker_label(vad_list)  # post process after vad

    logger.info("Step 4: ASR")
    segment_list = [{"start": 0, "end": len(audio["waveform"]) / audio["sample_rate"]}]
    asr_result = asr(segment_list, audio)
    ...
    logger.info("Step 5.2: Filter out files with less than average MOS")
    filtered_list = filter(mos_list, kwargs["min_dnsmos"])

    logger.info("Step 6: write result (MP3 and JSON to Webdataset)")
    json_data = json.load(io.BytesIO(sample["json"]))
    export_to_wds(writer, audio, filtered_list, audio_name, json_data)
...
if __name__ == "__main__":
    ...
    for sample in generate_sample(dataset, split_stereo=args.split_stereo):
        try:
            main_process_wds(sample, writer=writer, min_dnsmos=args.min_dnsmos)
