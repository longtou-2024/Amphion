...
IMAGE_URL = "us-central1-docker.pkg.dev/prod-ai-project/tts/amphion:v5.2"
N_GPU = 1
N_CPU = "12"
MEM_SIZE = "100Gi"
...
RECIPE_NAME = "iu_lbh"
INPUT_FOLDER_PATH = "gs://prod-ai-lab-speech-bucket/longtou/db/others/" + RECIPE_NAME + "/wds_tmp/shard-000000.tar"
INPUT_FOLDER_PATH2 = "gs://prod-ai-lab-speech-bucket/longtou/db/others/" + RECIPE_NAME + "/wds_tmp/shard-000001.tar"
GCS_URL = "gs://prod-ai-lab-speech-bucket/longtou/db/others/" + RECIPE_NAME + "/"
F_LOG = MOUNT_PATH + "/longtou/db/others/" + RECIPE_NAME +"/emilia_pipe_tmp_0.log"
F_LOG2 = MOUNT_PATH + "/longtou/db/others/" + RECIPE_NAME +"/emilia_pipe_tmp_1.log"
# --split_stereo
SHELL_COMMAND = f''' \
export CUDA_VISIBLE_DEVICES="0" \
&& . ./activate_python.sh \
&& python main_wds.py --config_path {CONFIG_PATH} --input_folder_path {INPUT_FOLDER_PATH} --wds_path {WDS_PATH} --gcs_url {GCS_URL} --f_log {F_LOG} --max_gpu_mem_frac 0.5 --start_shard 0 > /dev/null 2>&1 & sleep 10 && export CUDA_VISIBLE_DEVICES="0" && . ./activate_python.sh && python main_wds.py --config_path {CONFIG_PATH} --input_folder_path {INPUT_FOLDER_PATH2} --wds_path {WDS_PATH} --gcs_url {GCS_URL} --f_log {F_LOG2} --max_gpu_mem_frac 0.5 --start_shard 10
'''
